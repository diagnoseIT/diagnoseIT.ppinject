...
/**
 * Stores the activation state (including configuration) for a performance problem. This class corresponds to Kieker's
 * ProbeController.
 * 
 * @author Philipp Keck (University of Stuttgart)
 *
 * @param <C> The type of configuration that the problem implementation needs to store.
 */
public final class SignatureConfigurationStore<C extends ProblemConfiguration> {
     ...
     * 
     * @param sig an AspectJ Signature
    ...
    private static StringBuilder addTypeList(final StringBuilder sb, final Class<?>[] clazzes) {
        if (null != clazzes) {
            boolean first = true;
    ...
    private static StringBuilder addType(final StringBuilder sb, final Class<?> clazz) {
        if (null == clazz) {
            sb.append("ANONYMOUS");
     ...
     * 
     * @param pattern The pattern, which matches all signatures that shall be configured.
...
//    }
     ...
     * @return The corresponding configuration or <tt>null</tt>, if the signature is not configured (i.e. deactivated).
     */
